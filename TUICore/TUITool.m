...
+ (NSString *)convertDateToStr:(NSDate *)date
{
    ...
    NSDate *qianDay = [[NSDate alloc] initWithTimeIntervalSinceNow:- 2 * secondsPerDay];
    ...
    NSDateComponents *yesterCompoent = [calendar components:NSCalendarUnitDay|NSCalendarUnitMonth|NSCalendarUnitYear|NSCalendarUnitWeekOfMonth fromDate:yearsterDay];
    NSDateComponents *qianCompoent = [calendar components:NSCalendarUnitDay|NSCalendarUnitMonth|NSCalendarUnitYear|NSCalendarUnitWeekOfMonth fromDate:qianDay];
        ...
        if (nowComponent.month == dateCompoent.month) {
            ...
            if (nowComponent.weekOfMonth == dateCompoent.weekOfMonth) {
                ...
                if (nowComponent.day == dateCompoent.day) {
                    ...
                    // 离今天一周之内的显示，本周显示昨天，前天，星期
                    if (dateCompoent.year == yesterCompoent.year && dateCompoent.month == yesterCompoent.month && dateCompoent.day == yesterCompoent.day) {
                        //昨天
                        tipStr = @"昨天";
                        dateFmt.dateFormat = @"HH:mm";
                    } else if (dateCompoent.year == qianCompoent.year && dateCompoent.month == qianCompoent.month && dateCompoent.day == qianCompoent.day) {
                        //前天
                        tipStr = @"前天";
                        dateFmt.dateFormat = @"HH:mm";
                    } else {
    ...
    NSString *str = [NSString stringWithFormat:@"%@ %@", tipStr, [dateFmt stringFromDate:date]];
    return str;
}
