...
//    [self addSubview:_collectionView];
    ...
    self.selectLab.textColor = [UIColor colorWithHex:@"333333"];
    self.selectLab.font = [UIFont systemFontOfSize:18];
    NSString *selectMsg = @"已选择：0人";
    NSMutableAttributedString *att = [[NSMutableAttributedString alloc] initWithString:selectMsg];
    [att addAttributes:@{NSForegroundColorAttributeName: [UIColor colorWithHex:@"0x03BDAC"]} range:NSMakeRange(4, selectMsg.length - 4)];
    self.selectLab.attributedText = att;
...
//                                 forState:UIControlStateHighlighted];
    [self.accessoryBtn setBackgroundColor:[UIColor colorWithHex:@"0x03BDAC" alpha:0.4]];
    self.accessoryBtn.layer.cornerRadius = 4;
...
//        [self.collectionView reloadData];
        ...
        NSString *selectMsg = [NSString stringWithFormat:@"已选择：%ld人", newSelectArray.count];
        NSMutableAttributedString *att = [[NSMutableAttributedString alloc] initWithString:selectMsg];
        [att addAttributes:@{NSForegroundColorAttributeName: [UIColor colorWithHex:@"0x03BDAC"]} range:NSMakeRange(4, selectMsg.length - 4)];
        self.selectLab.attributedText = att;
        
        if ([newSelectArray isKindOfClass:NSArray.class]) {
            if ([newSelectArray count]) {
                self.accessoryBtn.enabled = YES;
                [self.accessoryBtn setBackgroundColor:[UIColor colorWithHex:@"0x03BDAC" alpha:1]];
            } else {
                self.accessoryBtn.enabled = NO;
                [self.accessoryBtn setBackgroundColor:[UIColor colorWithHex:@"0x03BDAC" alpha:0.4]];
            }
...
- (void)layoutSubviews
{
    ...
    self.accessoryBtn.mm_width(54).mm_height(30).mm_right(15).mm_top(13);
//    self.collectionView.mm_left(15).mm_height(40).mm_width(self.accessoryBtn.mm_x - 30).mm__centerY(self.accessoryBtn.mm_centerY);
    self.selectLab.mm_left(12).mm_height(25).mm_width(self.accessoryBtn.mm_x - 30).mm__centerY(self.accessoryBtn.mm_centerY);
}
...
- (void)tuitheme_addAction:(UIAlertAction *)action {
    if (action.style == UIAlertActionStyleDefault || action.style == UIAlertActionStyleCancel) {
        UIColor *tempColor = TUICoreDynamicColor(@"primary_theme_color", @"#03BDAC");
